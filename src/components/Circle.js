      ...
      <map name="image-map">
        {keys.map((key, index) => {
          return (
            <area
              key={index}
              onClick={(e) => renderMusic(e, key.title)}
              target="_blank"
              alt={key.alt}
              title={key.title}
              href={key.href}
        ...
        <area
          ...
          alt="G♭Major / e♭ minor"
          title="G♭Major / e♭ minor"
          data-flatio={`https://flat.io/embed/5fa0866ffdc8885b55a586b3?appId=${process.env.REACT_APP_FLATIO}`}
